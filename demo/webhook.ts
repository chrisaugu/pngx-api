...
const WEBHOOK_TOKEN = process.env.WEBHOOK_TOKEN || "your_shared_webhook_token";
...
const verifyWebhookMiddleware = (webhookToken: string, secret: string, timeDifference: number) => {
  return (req: Request, res: Response, next: NextFunction) => {
    const token = req.headers['x-webhook-token'];
    const signature = req.headers['x-signature'] as string;
    const timestamp = req.headers['x-timestamp'] as unknown as number;
...
router.post('/webhook', verifyWebhookMiddleware(WEBHOOK_TOKEN, SECRET, MAX_TIME_DIFFERENCE), (req, res) => {
...
router.post('/trigger-webhook', async (req, res) => {
  ...
  webhooks.forEach((webhook) => {
    if (webhook.events.includes(event)) {
      console.log(`Triggering webhook at ${webhook.url} with payload:`, payload);
      axios
...
function sendWebhook(webhook: any, payload: any) {
...
const sendWithRetry = async (url: string, payload: any, retries = 1, baseDelayMs = 1000) => {
  let attempt = 0;
  ...
  while (attempt <= retries) {
    try {
      ...
      console.log(`Successfully triggered webhook at ${url}: ${response.status}`);
      return true;
    } catch (err: any) {
...
function _generateSignature(secret: string, payload: string) {
  return crypto
...
function _verifySignature(secret: string, signature: string, payload: any) {
  const hash = crypto
    .createHmac('sha256', secret)
    .update(payload)
    .digest('hex');
...
const verifyWebhookTokenMiddleware = (webhookToken: string) => {
  return (req: Request, res: Response, next: NextFunction) => {
    const token = req.headers['x-webhook-token'];
